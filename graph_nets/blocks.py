...
class obsEncoderBlock(snt.AbstractModule):
  ...
  def __init__(self,
    ...
    super(obsEncoderBlock, self).__init__(name=name)
    with self._enter_variable_scope():
      self._node_model = encoder_fn()
  ...
  def _build(self, graph):
    ...
    nodes_to_collect.append(graph.obs)
    collected_nodes = tf.concat(nodes_to_collect, axis=-1)
    updated_nodes = self._node_model(collected_nodes)
...
class qEncoderBlock(snt.AbstractModule):
  ...
  def __init__(self,
    ...
    super(qEncoderBlock, self).__init__(name=name)
    with self._enter_variable_scope():
      # self._conv_model = conv_fn()
      self._mlp_model=mlp_fn()
  ...
  def _build(self, graph):
    ...
    nodes_to_collect.append(graph.nodes)
    nodes_to_collect.append(graph.lnodes)
    collected_nodes = tf.concat(nodes_to_collect, axis=-1)
    updated_nodes = self._mlp_model(collected_nodes) 
...
class LEdgeBlock(snt.AbstractModule):
  def __init__(self,
               ...
               name="ledge_block"):
    super(LEdgeBlock, self).__init__(name=name)

    if not (use_edges or use_sender_nodes or use_receiver_nodes or use_globals or use_reverse_edges):
      raise ValueError("At least one of use_edges, use_sender_nodes, "
                       "use_receiver_nodes or use_globals must be True.")
    ...
    self._use_sender_nodes = use_sender_nodes
    self._use_globals = use_globals
    self._use_reverse_edges= use_reverse_edges
    with self._enter_variable_scope():
      self._edge_model = edge_model_fn()
  ...
  def _build(self, graph):
    ...
    _validate_graph(
        graph, (LSENDERS, LRECEIVERS, N_EDGE), " when using an EdgeBlock")

    edges_to_collect = []

    if self._use_edges:
      _validate_graph(graph, (LEDGES,), "when use_edges == True")
      edges_to_collect.append(graph.ledges)
    if self._use_reverse_edges:
      edges_to_collect.append(graph.edges)
      # edges_to_collect.append(graph.edges)
      # edges_to_collect.append(graph.edges)
    ...
    if self._use_receiver_nodes:
      edges_to_collect.append(broadcast_lreceiver_nodes_to_ledges(graph))

    if self._use_sender_nodes:
      # remove self nodes influence
   #   edges_to_collect.append(broadcast_lsender_nodes_to_ledges(graph))
      edges_to_collect.append(tf.gather(graph.hnodes, graph.lsenders))
    else:
      edges_to_collect.append(tf.gather(graph.hnodes, graph.lsenders))
# if self._use_globals:
    #   edges_to_collect.append(broadcast_globals_to_edges(graph))
      ...
      if self._use_received_edges:
        if received_edges_reducer is None:
          raise ValueError(
              "If `use_received_edges==True`, `received_edges_reducer` "
              "should not be None.")
        self._received_edges_aggregator = LReceivedEdgesToNodesAggregator(
            received_edges_reducer)
...
class HEdgeBlock(snt.AbstractModule):
  ...
  def _build(self, graph):
    ...
    _validate_graph(
        graph, (HSENDERS, HRECEIVERS, N_HEDGE), " when using an EdgeBlock")

    edges_to_collect = []

    # if self._use_edges:
    #   _validate_graph(graph, (HEDGES,), "when use_edges == True")
    #   edges_to_collect.append(graph.hedges)

    # if self._use_receiver_nodes:
    #   edges_to_collect.append(broadcast_hreceiver_nodes_to_hedges(graph))
    #if self._use_received_edges:
    edges_to_collect.append(tf.gather(self._received_edges_aggregator(graph),graph.hsenders))

    #if self._use_sender_nodes:
    #import pdb;pdb.set_trace()
    edges_to_collect.append(broadcast_hsender_nodes_to_hedges(graph))
    collected_edges = tf.concat(edges_to_collect, axis=-1)
    updated_edges = self._edge_model(collected_edges)
      ...
      if self._use_received_edges:
        if received_edges_reducer is None:
          raise ValueError(
              "If `use_received_edges==True`, `received_edges_reducer` "
              "should not be None.")
        self._received_edges_aggregator = HReceivedEdgesToNodesAggregator(
            received_edges_reducer)
      # if self._use_sent_edges:
      #   if sent_edges_reducer is None:
      #     raise ValueError(
