...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str, choices={'ac', 'mfac', 'mfq', 'il','mfqc','ilqc','milc','millc','mill','cil','hil','gil'}, help='choose an algorithm from the preset', required=True)
    ...
    # main_model_dir = os.path.join(BASE_DIR, 'data/models/il-0')
    # oppo_model_dir = os.path.join(BASE_DIR, 'data/models/{}-1'.format(args.algo))

    oppo_model_dir = os.path.join(BASE_DIR, 'data/models/{}-1'.format(args.algo))
    main_msg_dir = os.path.join(BASE_DIR, 'data/models/{}-msg0'.format(args.algo))
    ...
    if args.idx!='None':
        ...
        if args.usemsg!='None':
    # models[0].load(main_model_dir, step="6-4selfcrpnw")
    ...
    runner = tools.Runner(sess, env, handles, args.map_size, args.max_steps, models,MsgModels ,play,
                            ...
                            log_dir=log_dir, model_dir=model_dir, train=True,len_nei=args.len_nei,rewardtype= args.rewardtype,crp=crp,is_selfplay=True,is_fix=False)

    # start_from=2000
    for k in range(start_from, start_from + args.n_round):
        eps = magent.utility.piecewise_decay(k, [0, 700, 1400,8000], [1, 0.3, 0.05,0.01])
        # eps=0   
        runner.run(eps, k)
