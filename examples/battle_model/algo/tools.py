...
class Runner(object):
    def __init__(self, sess, env, handles, map_size, max_steps, models,MsgModels,
                play_handle, render_every=None, save_every=None, tau=None, log_name=None, log_dir=None, model_dir=None, train=False,len_nei=40,rewardtype='self',crp=False,crps=[False,False],is_selfplay=False,is_fix=False):
        """Initialize runner
    ...
    def run(self, variant_eps, iteration, win_cnt=None,total_rewards=None):
        ...
        max_nums, nums, agent_r_records, total_rewards = self.play(env=self.env, n_round=iteration, map_size=self.map_size, max_steps=self.max_steps, handles=self.handles,
                    models=self.models, MsgModels=self.MsgModels,print_every=50, eps=variant_eps, render=(iteration + 1) % self.render_every if self.render_every > 0 else False, train=self.train,len_nei=self.len_nei,rewardtype=self.rewardtype,crp=self.crp,
...
#        info['main']['getkilled']=info['opponent']['kill']
        ...
        if self.train:
            ...
            if info['main']['total_reward'] > info['opponent']['total_reward'] and self.is_selfplay:
                ...
                print(Color.INFO.format('[INFO] Saving model ...'))
                self.update_cnt+=1
                #if self.update_cnt%7==0 and self.update_cnt > 300:
                #    self.models[0].save(self.model_dir + '-0', str(self.len_nei)+'-'+str(iteration%5)+self.rewardtype+('crp' if self.crp else 'nom')+('w' if self.MsgModels[0] else 'nw'))
                #    self.models[1].save(self.model_dir + '-2', str(self.len_nei)+'-'+str(iteration%5)+self.rewardtype+('crp' if self.crp else 'nom')+('w' if self.MsgModels[0] else 'nw'))
                #    if self.MsgModels[0]:
                #        self.MsgModels[0].save(self.model_dir + '-msg0', str(self.len_nei)+'-'+str(iteration%5)+self.rewardtype+('crp' if self.crp else 'nom')+'w')
                    #     self.MsgModels[1].save(self.model_dir + '-msg1', str(self.len_nei)+'-'+str(iteration%5)+self.rewardtype+('crp' if self.crp else 'nom')+'w')
                self.summary.write(info['main'], iteration)
    ...
    #,info['main']['kill'],info['main']['getkilled']
