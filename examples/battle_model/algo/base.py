...
class ValueNet:
    def __init__(self, sess, env, handle, name,len_nei, update_every=5, use_mf=False, learning_rate=1e-4, tau=0.99, gamma=0.95,num_bits_msg=3,msg_space=(40,40),use_msg=False,is_msg=False,use_mean_msg=False,is_hie=False,crp=False):
...
class GCrpNet:
    def __init__(self, sess, env, handle, name,len_nei=6, update_every=5,learning_rate=1e-4, tau=0.99, gamma=0.95,num_bits_msg=3,isHie=False,is_comm=False):
        # assert isinstance(env, GridWorld)
        self.env = env
        self.name = name
        self._saver = None
